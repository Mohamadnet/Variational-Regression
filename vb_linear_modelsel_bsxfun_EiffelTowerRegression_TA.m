...
% % % x_axis = 1963:2013;
% % % x_test_axis = [];
% % % x_test_axis = 1963:1/PointNum:1963+1;
% % % for i=1964:2012
% % %     temp = i:1/PointNum:i+1;
% % %     x_test_axis = [x_test_axis temp(2:end)];
% % % end
% % % x_test_axis_Pred = [];
% % % x_test_axis_Pred = 2013:1/PointNum:2013+1;
% % % for i=2014:2014+prediction_Year-1
% % %     temp = i:1/PointNum:i+1;
% % %     x_test_axis_Pred = [x_test_axis_Pred temp(2:end)];
% % % end
% % % 
% % % %separate train and test records
% % % prediction_record = [];
% % % prediction_record(1:PointNum,:) = x(end-PointNum:end-1,:);
% % % for i=1:prediction_Year
% % %     prediction_record(end+1:end+PointNum,:) = x(end-PointNum:end-1,:);
% % % end
% % % prediction_record(end+1,:) = x(end,:);
% % % x_test = [x_test;prediction_record];
...
% variational bayes
M = ones(size(train_record,1),1);
X_test = ones(size(test_record,1),1);
X_test_2 = ones(size(x_test,1),1);
for i=1:8
...
[w_VB, V_VB, ~, ~, an_VB, bn_VB] = vb_linear_fit_ard(M, y(train_record_num),1,0.1,10,1);
