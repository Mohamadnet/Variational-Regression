...
phi=@(a,n)(bsxfun(@minus,abs(bsxfun(@minus,a,linspace(-4,4,n))),linspace(-4,4,n))); 
%%Preprocessing
...
%% settings
N = [1 2 4 8 10 14 16 18];   % model selection experimental design
N_MLE = 1;
PointNum = 12;
k=1;
x_test = [];
...
for i = 1:length(N)
    ...
    [~, ~, ~, ~, ~, ~, ~, Ls(i)] = vb_linear_fit_ard(M, y(train_record_num),0.01,0.0001,0.01,0.0001);
