...
phi=@(a,n)(bsxfun(@times,legendre(13,a/n)',0.15.^[0:13]));
%%Preprocessing
% spline interpolation for Time-series data
...
%% settings
N = [8 12 16 20];   % model selection experimental design
N_MLE = 3;
PointNum = 12;
k=1;
x_test = [];
...
[w_VB, V_VB, ~, ~, an_VB, bn_VB] = vb_linear_fit(M, y(train_record_num) );
...
M = [];
M = phi(train_record(:,1),N_MLE);
X_test = [];
X_test = phi(test_record(:,1),N_MLE); 
X_test_2 = [];
X_test_2 = phi(x_test(:,1),N_MLE); 
for i=2:8
    M = [M phi(train_record(:,i),N_MLE)];
    X_test = [X_test phi(test_record(:,i),N_MLE)];
    X_test_2 = [X_test_2 phi(x_test(:,i),N_MLE)];
end
