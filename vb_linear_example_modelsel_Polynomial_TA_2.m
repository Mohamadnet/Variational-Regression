...
for i = 1:length(Ds)
    M = [M , train_record.^i];
    [~, ~, ~, ~, ~, ~, ~, Ls_train(i)] = vb_linear_fit(M, y(train_record_num));
end
[~, i] = max(Ls_train);
D_best_train = Ds(i);
...
X_test_2 = ones(size(x_test(:,1)));
for i = 1:D_best_train
    M = [M , train_record.^i];
    X_test = [X_test , test_record.^i]; 
    X_test_2 = [X_test_2 , x_test.^i]; 
end
[w_VB_test, V_VB_test, ~, ~, an_VB_test, bn_VB_test] = vb_linear_fit(M, y(train_record_num) );
