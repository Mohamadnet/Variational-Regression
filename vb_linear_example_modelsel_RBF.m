...
Ds = 1:10;
x_ramge_domain = 50;
x_range = [-x_ramge_domain x_ramge_domain];
RBF_size = 5;
...
x_test_RBF = [x_test_RBF x_test_RBF];
...
[xx1 xx2] = meshgrid(-x_ramge_domain:0.1:x_ramge_domain,-x_ramge_domain:0.1:x_ramge_domain);
%plot3(x_test(:,1)',x_test(:,2)', y_test, 'k-', 'LineWidth', 1);
plot3(x_test(:,1)',x_test(:,2)', y_VB, '--', 'Color', [0.8 0 0], 'LineWidth', 1);
plot3(x_test(:,1)',x_test(:,2)', y_ML, '-.', 'Color', [0 0 0.8], 'LineWidth', 1);
plot3(x(:,1)',x(:,2)', y, 'k+', 'MarkerSize', 5);
mesh(xx1,xx2,y_test)            % prediction point plot
grid ; view(3);
%set(gca, 'Box','off', 'PlotBoxAspectRatio', [4/3 1 1], ...
 %   'TickDir', 'out', 'TickLength', [1 1]*0.02/(4/3));
xlabel('x');
