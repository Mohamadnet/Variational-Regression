...
phi=@(a,n)([cos(bsxfun(@times,a,[0:n:4])),sin(bsxfun(@times,a,[-1:n:4]))]);
...
%% settings
N = [1/20 1/15 1/10 1/4 1/3 1/2 1 3/2 2 5/2];   % model selection experimental design
N_MLE = 1;
PointNum = 12;
k=1;
x_test = [];
...
for i = 1:length(N)
    ...
    [~, ~, ~, ~, ~, ~, ~, Ls(i)] = vb_linear_fit_ard(M, y(train_record_num),2,0.2,20,2);
...
[w_VB, V_VB, ~, ~, an_VB, bn_VB] = vb_linear_fit_ard(M, y(train_record_num),2,0.2,20,2);
